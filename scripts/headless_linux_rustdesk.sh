...
if [ ! -e "${CONF_FILE}" ]; then
    ...
    # Use a quoted heredoc to avoid variable expansion and suppress tee stdout
...
EOF
    ...
    # restart display manager (fallbacks in case display-manager unit isn't present)
    sudo systemctl restart display-manager || sudo systemctl restart gdm || sudo systemctl restart lightdm || true
    sleep 2
    sudo systemctl restart nxserver || true
    sleep 1
fi
